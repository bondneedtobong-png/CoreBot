...
|----------|:---:|:---:|
| ~~**Дублирование создания DB-сессии**~~ | — | ✅ Переведено на `session_scope()` |
| ~~**Конфликт callback `cancel`**~~ | — | ✅ Разведены по разделам + legacy в `main.py` |
| ~~**Рассылка / один аккаунт, нет балансировки**~~ | — | ✅ Есть ротация аккаунтов по `messages_per_batch` |
| ~~**`typing_enabled` захардкожен** в `start_mailing()`~~ | — | ✅ Используется флаг рассылки `use_typing` |
| **Pytest-покрытие базовое** (нет полного регрессионного набора) | 🟡 | 🛠 В процессе |
| ~~**Graceful shutdown неполный**~~ (без stop активной рассылки) | — | ✅ Добавлен stop рассылки и disconnect воркеров при завершении |
| **Пагинация частично закрыта** (добавлена для клиентов и рассылок; возможны доп. экраны) | 🟡 | 🛠 В процессе |
