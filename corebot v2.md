...
- Статус: `in progress`
- Активный этап: `Этап 6 (web-panel) — закрытие CRUD-gaps`
- Ближайшая цель: расширение регрессионных тестов вокруг классов и очереди (outbound queue retry / mailing lock RUNNING).
...
- [ ] Минимальный мониторинг внутри `Нейрочаттинг` (счётчики причин deny за последние N часов).
