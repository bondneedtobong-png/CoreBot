...
PLAYBOOKS = {
    name: FLEET / "playbooks" / f"{name}.yml"
    for name in ("install", "update", "verify", "backup", "rollback")
}
...
def test_inventory_two_instances_without_playbook_copy():
    ...
    allowed = {
        ...
        "owner_id",
        "corebot_app_dir",
        "corebot_database_url",
        "corebot_bot_database_url",
        "corebot_cp_database_url",
    }
    for name, vars_ in hosts.items():
        ...
        for key in vars_:
            assert key in allowed, (
                f"{name}: non-inventory key {key} (no playbook copy, no secrets)"
            )
            assert not any(s in key.lower() for s in SECRET_KEYS), (
                f"{name}: secret key in inventory: {key}"
            )
        ...
        assert str(vars_["ansible_host"]).endswith(".invalid"), (
            f"{name}: host must be .invalid placeholder"
        )
    ...
    assert REAL_TOKEN_RE.search(dumped) is None, (
        "secret-like token in inventory example"
    )
...
def test_dotenv_template_flat_no_expansion_per_host_differs():
    ...
    for out in (out_a, out_b):
        for line in out.splitlines():
            ...
            assert not stripped.lower().startswith("export "), (
                "no export prefix allowed"
            )
...
def test_rendered_dotenv_passes_production_validator(tmp_path):
    ...
    assert result.ok, (
        f"fleet-rendered .env must pass production validation: {result.errors}"
    )
...
def test_vault_example_schema_no_plaintext_secrets():
    ...
    assert (
        isinstance(data.get("vault_instances"), dict)
        and len(data["vault_instances"]) >= 2
    )
    ...
    for value in _walk_scalars(data):
        if not value.strip():
            continue
        assert value.startswith("VAULT:"), (
            f"vault example value must be VAULT: placeholder, got {value!r}"
        )
        assert REAL_TOKEN_RE.search(value) is None, (
            f"secret-like token in vault example: {value!r}"
        )
        assert HEX64_RE.search(value) is None, (
            f"real-looking hex secret in vault example: {value!r}"
        )
...
def simulate_canary_rollout(
    hosts: list[str], failed: set[str], serial: list
) -> list[str]:
...
def test_update_canary_blocks_rest_on_failure():
    ...
    assert simulate_canary_rollout(hosts, {"corebot-test-alpha"}, play["serial"]) == [
        "corebot-test-alpha"
    ]
...
def test_no_log_on_secret_tasks():
    secret_ref = re.compile(
        r"vault_|instance_bot_token|instance_cp_jwt|instance_cp_admin|bot_token|jwt_secret|admin_password|agent_token",
        re.IGNORECASE,
    )
    checked = 0
    for path in list((FLEET / "playbooks").glob("*.yml")) + list(
        (FLEET / "roles").rglob("tasks/*.yml")
    ):
...
def test_report_template_has_no_secrets_and_covers_contract():
    ...
    assert not re.search(
        r"bot_token|jwt_secret|admin_password|api_hash|agent_token", raw, re.IGNORECASE
...
def test_idempotency_markers_and_check_mode_support():
    tasks_text = "\n".join(
        (p.read_text(encoding="utf-8") for p in (FLEET / "roles").rglob("tasks/*.yml"))
    )
    ...
    assert "changed_when: false" in tasks_text, (
        "read-only tasks must declare changed_when: false"
    )
...
def test_all_five_playbooks_exist_with_limit_support():
    for name, path in PLAYBOOKS.items():
        ...
        assert _play_hosts(plays[0]) == ["corebot_fleet"], (
            f"{name}: must target corebot_fleet (use --limit for one host)"
        )
...
def test_ansible_cfg_keeps_host_key_checking_and_local_vault():
    ...
    assert parser.getboolean("defaults", "host_key_checking") is True, (
        "host key checking must stay ON"
    )
    vault_id = parser.get("defaults", "vault_identity_list")
    assert "~" in vault_id or ".ansible" in vault_id, (
        "vault password must live outside the repo"
    )
    assert "ops/" not in vault_id and "group_vars" not in vault_id
    raw = ANSIBLE_CFG.read_text(encoding="utf-8")
    assert (
        "host_key_checking = False" not in raw and "host_key_checking=False" not in raw
    )
...
def test_vault_file_ignored_and_example_only_in_git():
    ...
    assert not (FLEET / "group_vars" / "all" / "vault.yml").exists(), (
        "real vault file must not exist in repo"
    )
    assert not (FLEET / "inventory" / "hosts.yml").exists(), (
        "real inventory must not exist in repo"
    )
