...
5. TData-check пул отдельно от runtime: проверочные прокси живут только в
   группах с назначением TDATA_CHECK (бот: `Прокси → Массово в группу →
   TData-check`; панель: `POST /business/proxy-groups` с
   `purpose=TDATA_CHECK`). Лимиты проверки: архив до 200 МБ, до 20 папок,
   concurrency 3, timeout 25 с, tmp чистится сразу, FloodWait — новым
   запуском после паузы.
