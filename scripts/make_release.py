...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate CoreBot RELEASE.json manifest.")
    parser.add_argument("--root", default=str(Path(__file__).resolve().parent.parent),
                        help="code tree to describe (default: repo root)")
    parser.add_argument("--output", default=None, help="manifest path (default: <root>/RELEASE.json)")
    parser.add_argument("--sha", default=None, help="explicit full git SHA (default: git HEAD)")
    parser.add_argument("--print", dest="do_print", action="store_true", help="print manifest JSON")
    args = parser.parse_args(argv)

    manifest = build_manifest(args.root, sha=args.sha)
    errors = verify_manifest(manifest, args.root if manifest["sha"] != "unknown" else None)
    if errors:
        for err in errors:
